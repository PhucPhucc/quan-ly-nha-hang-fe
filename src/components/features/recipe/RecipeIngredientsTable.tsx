      ...
      <Table>
        <TableHeader className="bg-neutral-50/30">
          <TableRow className="border-none">
            ...
            <TableHead className="text-center">{UI_TEXT.MENU.RECIPE.STOCK_COLUMN}</TableHead>
        ...
        <TableBody>
          {fields.map((field, index) => {
            const currentStock = normalizeInventoryQuantity(
              watchedIngredients[index]?.currentStock ?? 0
            );
            const quantity = normalizeInventoryQuantity(watchedIngredients[index]?.quantity ?? 0);
            const isExceeding = currentStock > 0 && quantity > currentStock;
            ...
            return (
              ...
              >
                ...
                <TableCell>
                  <div className="flex items-center justify-center gap-2 max-w-[150px] mx-auto">
                    <Input
                      type="number"
                      step="0.001"
                      className={`h-9 text-center focus:ring-primary/20 ${isExceeding ? "border-destructive focus:border-destructive" : ""}`}
                  ...
                  {isExceeding && (
                    <p className="text-[10px] text-destructive mt-1 text-center flex items-center justify-center gap-1">
                      <AlertTriangle className="w-3 h-3" />
                      {UI_TEXT.MENU.RECIPE.EXCEED_STOCK}
                    </p>
                  )}
                </TableCell>
                <TableCell className="text-center">
                  <span
                    className={`text-sm ${
                      currentStock <= 0 ? "text-destructive font-semibold" : "text-muted-foreground"
                    }`}
                  >
                    {currentStock} {field.unit}
                  </span>
                </TableCell>
