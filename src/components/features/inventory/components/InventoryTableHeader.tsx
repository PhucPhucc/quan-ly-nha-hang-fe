...
export function InventoryTableHeader({
  ...
  return (
    <div className="w-full rounded-xl border border-border bg-card shadow-soft px-3 py-2 grid gap-2 lg:grid-cols-[1fr_auto_auto_auto] lg:items-center">
      <div className="flex-1 min-w-0">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <Input
            placeholder={UI_TEXT.INVENTORY.TOOLBAR.SEARCH_PLACEHOLDER}
            className="pl-9 h-9 rounded-xl bg-background text-foreground placeholder:text-muted-foreground border border-border focus-visible:ring-2 focus-visible:ring-muted"
            value={searchQuery}
      ...
      <div className="flex items-center gap-2 justify-end">
        <Select value={statusFilter} onValueChange={(v) => onStatusChange(v as StatusFilter)}>
          <SelectTrigger className="h-9 min-h-[36px] w-full sm:w-[180px] rounded-xl bg-background border border-border text-foreground">
