...
export const setMenuService = {
  getSetMenus: (
    ...
    return apiFetch<PaginationResult<SetMenu>>(`/v1/setmenus${query ? `?${query}` : ""}`);
  },

  getSetMenuById: (id: string): Promise<ApiResponse<SetMenu>> =>
    apiFetch<SetMenu>(`/v1/setmenus/${id}`),

  createSetMenu: (data: Partial<SetMenu>): Promise<ApiResponse<SetMenu>> =>
    apiFetch<SetMenu>("/v1/setmenus", {
      method: "POST",
      body: data,
    }),
